...
## 56. Vocab table "bolong" (~9-148 kata dasar ilang) — AUDITED, FALSE ALARM, `public.vocab` CLOSED

**Catatan penomoran**: nomor ini sempat dipakai sesi sebelumnya buat entry `reading_mc`/`image_tf`
convention (di bawah, sekarang jadi #57) — direnumber biar nggak nabrak, karena Kyaru sendiri
udah nyebut "vocab gap = PR #56" duluan pas nutup sesi kemarin.

Kecurigaan awal (dari audit kosakata-per-level pilot HSK1-2, sesi sebelumnya): `public.vocab`
(4991 baris) kurang dari "spek resmi 5000" HSK 2.0, dan kata dasar kayak 男/女/说/见/电话/英语/
篮球/笔 nol di seluruh tabel — dicurigai bug import atau data yang ke-drop.

**Audit source-verified (bukan LLM-generated)**: source asli ketemu LOKAL di repo —
`data/complete.json` (11.470 entri), dirujuk eksplisit di `sql/01_vocab_schema.sql` ("Sumber:
data/complete.json, difilter cuma level 'old-1'..'old-6' (HSK 2.0), BUKAN new-*/newest-* (HSK
3.0)"), struktur field match persis sama JSON yang di-embed `sql/02_vocab_import.sql`.

**Hasil diff (dedup per hanzi per level, bukan raw tag count)**: `old-1..old-6` di source =
150/147/298/598/1298/2500 — **match persis** `public.vocab`. **NOL kata hilang di semua level.**
`public.vocab` adalah salinan LENGKAP dan AKURAT dari tagging `old-*` source-nya sendiri — nol
bug import.

**Kenapa 男/女/电话/英语/篮球 dkk nol di tabel**: dicek di source, kata-kata ini **cuma punya tag
`new-*`/`newest-*` (HSK 3.0-era), NOL tag `old-*` (HSK 2.0)** — source-nya sendiri emang nggak
nganggep ini bagian skema 2.0. Cross-check 159 kandidat dari `vocab_gap_candidates.csv` sesi
lalu: 0/159 punya tag `old-*` (nol yang bisa direstore dari source yang sama), 90/159 cuma
new-*/newest-* (source sengaja nggak masukin ke 2.0), 69/159 nol di source sama sekali
(kebanyakan frasa majemuk 2-3 karakter, bukan headword tunggal).

**Konfirmasi Kyaru (diff internal + cross-check eksternal sendiri)**: tagging `old-*` di source
ini AKURAT — HSK 2.0 asli beneran cuma punya bentuk majemuk (bukan kata dasar tunggal) buat
sebagian kata-kata ini. Premis awal ("vocab bolong") **salah** — dibandingin ke folklore "5000"
generik, bukan ke source file yang sebenernya dipakai.

**Verdict: `public.vocab` CLOSED, nol tindakan.** Nol insert dilakukan (`missing_words_
category_A.csv`, 0 baris, dihapus). Kalau ada pertanyaan "kok X nggak ada di vocab" muncul lagi
nanti — rujuk ke sini dulu sebelum diasumsikan bug.

Diputuskan Kyaru + Claude Code, 19 Jul 2026.

## 57. Konvensi `reading_mc` dipakai sebagai wadah TF-teks (判断对错), `image_tf` cuma buat TF-bergambar — SENGAJA, JANGAN migrasi
...
## 58. Native `alert()`/`confirm()` diganti modal HTML bertema (cream/navy/gold) — reuse modal shell #31

Ditemuin Kyaru (polish, bukan bug kritis): submit mock test dengan soal belum lengkap masih pake
`confirm()` browser bawaan ("xingmandarin.com says") — nggak sesuai tema app. Audit ketemu 22
pemakaian `alert(` dan 2 pemakaian `confirm(` di `index.html`, semuanya di jalur user-facing
(bukan admin panel).

**Implementasi**: `showAlertModal(message, title)` dan `showConfirmModal(message, title)` (return
`Promise<boolean>`) — reuse `openModal()`/`closeModal()`/`.modalOverlay`/`.modalPanel`/`.modalBtn`
yang udah ada (dibangun buat admin panel, #31), nol komponen baru. 22 `alert(` → `showAlertModal(`
(name swap), 2 `confirm(` → `await showConfirmModal(` (+ `exitAttempt()` jadi `async`). Label
OK/Cancel di-i18n 3 bahasa (`modalOk`/`modalCancel`, ID/EN/ZH) karena 2 lokasi confirm() ada di
alur user-facing (submit/exit ujian), beda dari modal admin yang tetap hardcoded English (utang
lama, di luar scope).

**Fix kritis — `closeModal()` resolve-false**: `showConfirmModal()`'s Promise cuma di-resolve
manual lewat klik tombol OK/Cancel — Esc/backdrop-click/tombol X nutup modal lewat listener global
yang nggak tau soal Promise itu sama sekali, jadi kalau nggak di-handle, Promise itu ngegantung
selamanya (freeze diem-diem tiap kali user nutup confirm modal pake Esc, bukan klik tombol).
Di-fix dengan nambahin `modalPendingResolve` state di `closeModal()`: kalau ada Promise yang lagi
outstanding pas modal ditutup lewat jalur manapun, di-resolve `false` di situ juga. No-op buat
semua pemakaian `openModal()` lama (Edit user/Deactivate/Create) yang nggak pernah nyetel state
itu.

**Verifikasi (localhost, 6 test A-F)**: submit kosong (modal muncul, OK jalan, Cancel batal),
exit dengan jawaban terisi (Cancel jaga jawaban utuh), **Esc + backdrop-click saat confirm modal
kebuka — keduanya nutup bersih & CANCEL, nggak freeze, nggak ke-submit/ke-exit diem-diem** (ini
test paling kritis, buktiin fix di atas jalan), alert error (network di-inject gagal → modal
bertema muncul, bukan native), ganti bahasa ID/ZH → label ikut, console nol error di semua
langkah. Verifikasi live production abis push (submit kosong → modal bertema, bukan native
dialog).
